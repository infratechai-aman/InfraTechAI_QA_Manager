...
import { Plus, Briefcase, FolderOpen, ChevronRight, Trash2, Calendar, FileText, Sparkles, Check, X, Shield } from 'lucide-react';
...
export const ProjectsView = ({ 
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-8 animate-fadeIn">
      ...
      {/* Pending Workspace Invitations Card (Always prominently visible if user has pending invites) */}
        ...
        <div className="bg-gradient-to-r from-indigo-950 via-indigo-900 to-slate-900 text-white p-6 rounded-3xl border-2 border-indigo-500/40 shadow-xl space-y-4 animate-fadeIn">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-indigo-200">
                  Accept below to immediately join and collaborate on testing:
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-1">
            {pendingInvitations.map((inv) => (
              <div 
                key={inv.id} 
                className="bg-white/10 backdrop-blur-md border border-white/15 rounded-2xl p-5 flex flex-col justify-between gap-4 shadow-sm hover:border-indigo-400/50 transition-all"
              >
                <div>
                  <div className="flex items-start justify-between gap-2">
                    <h3 className="font-black text-white text-lg tracking-tight">
                      {inv.projectName || 'QA Workspace'}
                    </h3>
                  ...
                  <p className="text-xs text-indigo-200 mt-2">
                    Invited by: <strong className="text-white underline underline-offset-2">{inv.inviterEmail || 'Workspace Owner'}</strong>
                  </p>
                </div>
                ...
                <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-white/10">
                  {onDeclineInvitation && (
                    <button
                      ...
                      className="px-3 py-2 text-xs font-semibold text-slate-300 hover:text-white rounded-xl hover:bg-white/10 transition-colors cursor-pointer"
                    >
                      Decline
                    </button>
                  )}
                  {onAcceptInvitation && (
                    <button
                      ...
                      className="px-5 py-2.5 bg-emerald-500 hover:bg-emerald-600 active:scale-95 text-white font-black rounded-xl text-xs flex items-center gap-1.5 shadow-lg shadow-emerald-950/40 hover:scale-105 transition-all cursor-pointer"
                    >
